...
Tokenizer::Tokenizer(std::string path_to_bib_file)
{
    ...
    if (std::next(bib_file_.begin()) != bib_file_.end())
    {
...
std::vector<Token> Tokenizer::collect_next_entry_tokens()
{
    ...
    while (*tokenizer_buffer_.look_ahead != '@')
    {
        ...
        if (current_token_.type_ == EMPTY || current_token_.type_ == NEW_LINE)
        {
            // Do not include empty tokens and new line tokens in the output.
            continue;
        }
